...
function extractFromSwapEvent(
  tx: HeliusTransaction,
  swap: NonNullable<HeliusTransaction['events']>['swap'] & {},
  walletAddress: string,
  walletLabel: string
): DetectedSwap | null {
  const isSolMint = (mint: string) =>
    mint === CONFIG.SOL_MINT || mint === CONFIG.WSOL_MINT;
  ...
  // Token-to-token swap: check if any side is SOL/WSOL
  if (swap.tokenInputs.length > 0 && swap.tokenOutputs.length > 0) {
    ...
    if (solInput && tokenOutput) {
      // BUY via wrapped SOL
      const decimals = solInput.rawTokenAmount.decimals;
      const solAmount = parseInt(solInput.rawTokenAmount.tokenAmount) / Math.pow(10, decimals);
      const tDecimals = tokenOutput.rawTokenAmount.decimals;
      const tokenAmount = parseInt(tokenOutput.rawTokenAmount.tokenAmount) / Math.pow(10, tDecimals);
    ...
    if (tokenInput && solOutput) {
      // SELL via wrapped SOL
      const tDecimals = tokenInput.rawTokenAmount.decimals;
      const tokenAmount = parseInt(tokenInput.rawTokenAmount.tokenAmount) / Math.pow(10, tDecimals);
      const decimals = solOutput.rawTokenAmount.decimals;
      const solAmount = parseInt(solOutput.rawTokenAmount.tokenAmount) / Math.pow(10, decimals);
...
function extractFromTransfers(
  ...
  // Find token transfers involving this wallet
...
// ── Get recent swap transactions for a token (used by seeder) ──
...
export async function getTokenRecentBuyers(tokenMint: string): Promise<string[]> {
  ...
  for (let i = 0; i < signatures.length; i += CONFIG.TX_PARSE_BATCH_SIZE) {
    batches.push(signatures.slice(i, i + CONFIG.TX_PARSE_BATCH_SIZE));
  }
