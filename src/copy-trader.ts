...
export class CopyTrader {
  ...
  // ── Handle detected swap from a watched wallet ──
  ...
  private async handleBuy(swap: DetectedSwap): Promise<void> {
    ...
    if (!this.canTrade()) {
      log.warn(MODULE, `Cannot trade — ${this.openPositionCount}/${CONFIG.MAX_CONCURRENT_TRADES} positions, $${this.state.budgetRemaining.toFixed(2)} budget`);
      return;
    }
    ...
    log.trade(
      ...
      `Size: $${sizeUsd.toFixed(2)} | Price: $${this.fmtPrice(priceUsd)} | ` +
  ...
  private async handleSell(swap: DetectedSwap): Promise<void> {
    ...
    // Only copy sell from the same wallet that triggered our buy
    if (position.copiedWallet !== swap.walletAddress) {
      log.info(MODULE, `${swap.walletLabel} sold ${swap.tokenSymbol} but we copied from ${position.copiedWalletLabel} — ignoring`);
      return;
    }
  ...
  private async closePosition(position: CopyPosition, reason: string): Promise<void> {
    ...
    const holdTime = this.formatHoldTime(position.exitTime - position.entryTime);
    const sign = pnlUsd >= 0 ? '+' : '';
  ...
  printStatus(): void {
    ...
    console.log(`  Trades:          ${totalTrades} total | ${closed} closed (W: ${this.state.wins} / L: ${this.state.losses} | ${winRate.toFixed(0)}%)`);
    ...
    if (open.length > 0) {
      ...
      for (const p of open) {
        ...
        console.log(
          ...
          `Entry: $${this.fmtPrice(p.entryPriceUsd)} | Now: $${this.fmtPrice(p.currentPriceUsd)} | ` +
          `PNL: ${sign}${p.pnlPct.toFixed(1)}% | Hold: ${holdTime}`
        );
      }
    }
