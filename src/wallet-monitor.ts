...
export class WalletMonitor {
  ...
  // Callback when a swap is detected
  ...
  private async pollAllWallets(): Promise<void> {
    ...
    // Poll wallets sequentially to avoid rate limiting
  ...
  private async pollWallet(wallet: WatchedWallet): Promise<void> {
    ...
    // On first scan, just mark all existing signatures as seen (don't copy old trades)
    if (!this.initialScanDone.has(wallet.address)) {
      ...
      log.info(MODULE, `${wallet.label} — Initial scan done, marked ${sigs.length} existing transactions`);
      return;
    }
    ...
    log.info(MODULE, `${wallet.label} — ${newSigs.length} new transaction(s) detected`);
    ...
    for (let i = 0; i < signatures.length; i += CONFIG.TX_PARSE_BATCH_SIZE) {
      ...
      for (const tx of parsed) {
        ...
        log.trade(
          ...
          `Price: $${this.fmtPrice(swap.priceUsd)} | ` +
  ...
  // ── Garbage collect old signatures to prevent memory bloat ──
